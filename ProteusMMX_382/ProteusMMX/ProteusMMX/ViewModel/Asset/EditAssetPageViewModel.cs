...
{
    public class EditAssetPageViewModel : ViewModelBase, IHandleViewAppearing, IHandleViewDisappearing
    {
        #region Fields
        ...
        bool _editAssetIsEnabled = true;
        public bool EditAssetIsEnabled
        {
            get
            {
                return _editAssetIsEnabled;
            }

            set
            {
                if (value != _editAssetIsEnabled)
                {
                    _editAssetIsEnabled = value;
                    OnPropertyChanged(nameof(EditAssetIsEnabled));
                }
            }
        }
        bool _editAssetIsVisible = true;
        public bool EditAssetIsVisible
        {
            get
            {
                return _editAssetIsVisible;
            }

            set
            {
                if (value != _editAssetIsVisible)
                {
                    _editAssetIsVisible = value;
                    OnPropertyChanged(nameof(EditAssetIsVisible));
                }
            }
        }
        ...
        //Save Command
        public ICommand SaveAssetCommand => new AsyncCommand(EditAsset);
        ...
        public override async Task InitializeAsync(object navigationData)
        {
            try
            {
               // UserDialogs.Instance.ShowLoading();
                ...
                if (Application.Current.Properties.ContainsKey("IssueWorkorderKey"))
                {
                    Issue = Application.Current.Properties["IssueWorkorderKey"].ToString();

                }
                if (Application.Current.Properties.ContainsKey("EditAssetKey"))
                {
                    Edit = Application.Current.Properties["EditAssetKey"].ToString();
           ...
           if (Edit == "E")
            {
                this.EditAssetIsVisible = true;
            }
            else if (Edit == "V")
            {
                this.EditAssetIsEnabled = false;
            }
            else
            {
                this.EditAssetIsVisible = false;
            }
        ...
        public EditAssetPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IAssetModuleService assetService, IWorkorderService workorderService)
        {
        ...
        public async Task SetTitlesPropertiesForPage()
        {
            try
            {
                ...
                PageTitle = WebControlTitle.GetTargetNameByTitleName("Asset")+ WebControlTitle.GetTargetNameByTitleName("Details");
        ...
        private void GenerateTextBoxLayout(FormControl formControl, Grid contentGrid, int row, int column)
        {
            ...
            if (formControl.ControlName == "AssetName" || formControl.ControlName == "AssetNumber")
            {
                UserDialogs.Instance.HideLoading();
        ...
        private void GenerateComboBoxLayout(FormControl formControl, Grid contentGrid, int row, int column)
        {
            ...
            if (formControl.ControlName == "AssetSystemID")
            {
                control.IsVisible = false;
                UserDialogs.Instance.HideLoading();
                ...
                case "InstallationDate":
                    {
                        if (control is Picker)
                        {
                            ...
                            {
                                x.SelectedItem = item;
                                InstallationDate = item.SelectedValue.ToString();
                            }
               ...
               // UserDialogs.Instance.ShowLoading();
                ...
                if (Issue=="E")
                {
                    ...
                    if (response == CreateWorkorderTitle)
                    {
                        ...
                        tnobj.FacilityID = this.FacilityID;
                        tnobj.FacilityName = this.FacilityName;
                        tnobj.LocationID = this.LocationID;
        ...
        public async Task SetControlsPropertiesForPage()
        {
            ...
            if (AssetWrapper != null && AssetWrapper.assetWrapper != null && AssetWrapper.assetWrapper.asset != null)
            {
                ...
                {
                    AssetNameText = ShortString.shorten(asset.AssetName);
                }
                else
                {
                    AssetNameText = asset.AssetName;
                }
                AssetNumberText = asset.AssetNumber;
                if (!string.IsNullOrEmpty(asset.FacilityName))
                {
                    FacilityName = ShortString.shorten(asset.FacilityName);
                }
                else
                {
                ...
                {
                    LocationName = ShortString.shorten(asset.LocationName);
                }
                else
                {
                ...
                if (!string.IsNullOrEmpty(asset.AssetSystemName))
                {
                    AssetSystemName = ShortString.shorten(asset.AssetSystemName);
                }
                else
                {
                    AssetSystemName = asset.AssetSystemName;
                }
                AssetSystemID = asset.AssetSystemID;
                if (!string.IsNullOrEmpty(asset.CategoryName))
                {
                    CategoryText = ShortString.shorten(asset.CategoryName);
                }
                else
                {
                    CategoryText = asset.CategoryName;
                }
                CategoryID = asset.CategoryID;
                if (!string.IsNullOrEmpty(asset.VendorName))
                {
                    VendorText = ShortString.shorten(asset.VendorName);
                }
                else
                {
                    VendorText = asset.VendorName;
                }
                ...
                OriginalCost = string.Format(StringFormat.CurrencyZero(), string.IsNullOrWhiteSpace(asset.OriginalCost.ToString()) ? 0 : decimal.Parse(asset.OriginalCost.ToString()));
                Rating = asset.Rating;
                ...
                CurrentRuntimeText=string.Format(StringFormat.NumericZero(), string.IsNullOrWhiteSpace(asset.CurrentRuntime.ToString()) ? 0 : decimal.Parse(asset.CurrentRuntime.ToString()));
                DailyRuntime = string.Format(StringFormat.NumericZero(), string.IsNullOrWhiteSpace(asset.DailyRuntime.ToString()) ? 0 : decimal.Parse(asset.DailyRuntime.ToString()));

                ///Set Dyanmic Field Properties
                ///
                #region Set Dyanmic Field Properties

                LotoUrl = asset.LOTOUrl;

                #region User Fields

                UserField1 = asset.UserField1;
                UserField2 = asset.UserField2;
                UserField3 = asset.UserField3;
                UserField4 = asset.UserField4;
                UserField5 = asset.UserField5;
                UserField6 = asset.UserField6;
                UserField7 = asset.UserField7;
                UserField8 = asset.UserField8;
                UserField9 = asset.UserField9;
                UserField10 = asset.UserField10;
                UserField11 = asset.UserField11;
                UserField12 = asset.UserField12;
                UserField13 = asset.UserField13;
                UserField14 = asset.UserField14;
                UserField15 = asset.UserField15;
                UserField16 = asset.UserField16;
                UserField17 = asset.UserField17;
                UserField18 = asset.UserField18;
                UserField19 = asset.UserField19;
                UserField20 = asset.UserField20;
                UserField21 = asset.UserField21;
                UserField22 = asset.UserField22;
                UserField23 = asset.UserField23;
                UserField24 = asset.UserField24;
                #endregion
        ...
        private void OnCategoryRequested(object obj)
        {
            ...
            if (obj != null)
            {
                ...
                this.CategoryText = assignTo.SelectedText;
            }
        ...
        private void OnVendorRequested(object obj)
        {
            ...
            if (obj != null)
            {
                ...
                this.VendorText = requester.SelectedText;
            }
        ...
        private void OnRuntimeUnitRequested(object obj)
        {
            ...
            if (obj != null)
            {
                ...
                this.RuntimeUnitText = costCenter.SelectedText;
            }
        ...
        public async Task EditAsset()
        {
            try
            {
              ...
              RegexOptions.IgnoreCase);
                ...
                if (string.IsNullOrWhiteSpace(OriginalCost))
                {
                    OriginalCost = "0.0";
                }
                ...
                asset.UserField1 = String.IsNullOrEmpty(UserField1) ? null : UserField1.Trim();
                asset.UserField2 = String.IsNullOrEmpty(UserField2) ? null : UserField2.Trim();
                asset.UserField3 = String.IsNullOrEmpty(UserField3) ? null : UserField3.Trim();
                asset.UserField4 = String.IsNullOrEmpty(UserField4) ? null : UserField4.Trim();
                asset.UserField5 = String.IsNullOrEmpty(UserField5) ? null : UserField5.Trim();
                asset.UserField6 = String.IsNullOrEmpty(UserField6) ? null : UserField6.Trim();
                asset.UserField7 = String.IsNullOrEmpty(UserField7) ? null : UserField7.Trim();
                asset.UserField8 = String.IsNullOrEmpty(UserField8) ? null : UserField8.Trim();
                asset.UserField9 = String.IsNullOrEmpty(UserField9) ? null : UserField9.Trim();
                asset.UserField10 = String.IsNullOrEmpty(UserField10) ? null : UserField10.Trim();
                asset.UserField11 = String.IsNullOrEmpty(UserField11) ? null : UserField11.Trim();
                asset.UserField12 = String.IsNullOrEmpty(UserField12) ? null : UserField12.Trim();
                asset.UserField13 = String.IsNullOrEmpty(UserField13) ? null : UserField13.Trim();
                asset.UserField14 = String.IsNullOrEmpty(UserField14) ? null : UserField14.Trim();
                asset.UserField15 = String.IsNullOrEmpty(UserField15) ? null : UserField15.Trim();
                asset.UserField16 = String.IsNullOrEmpty(UserField16) ? null : UserField16.Trim();
                asset.UserField17 = String.IsNullOrEmpty(UserField17) ? null : UserField17.Trim();
                asset.UserField18 = String.IsNullOrEmpty(UserField18) ? null : UserField18.Trim();
                asset.UserField19 = String.IsNullOrEmpty(UserField19) ? null : UserField19.Trim();
                asset.UserField20 = String.IsNullOrEmpty(UserField20) ? null : UserField20.Trim();
                asset.UserField21 = String.IsNullOrEmpty(UserField21) ? null : UserField21.Trim();
                asset.UserField22 = String.IsNullOrEmpty(UserField22) ? null : UserField22.Trim();
                asset.UserField23 = String.IsNullOrEmpty(UserField23) ? null : UserField23.Trim();
                asset.UserField24 = String.IsNullOrEmpty(UserField24) ? null : UserField24.Trim();
                #endregion
                ...
                var response = await _assetService.EditAsset(assetwrapper);
                if (response != null && bool.Parse(response.servicestatus))
                {
                    DialogService.ShowToast(WebControlTitle.GetTargetNameByTitleName("Assetupdatedsuccessfully"), 2000);
            ...
            if (CustomControls != null && CustomControls.Count > 0)
            {
                foreach (var formLoadItem in CustomControls)
                {
                    switch (formLoadItem.ControlName)
                    {
                        ...
                        case "OriginalCostID":
                            {
                                validationResult = ValidateValidations(formLoadItem, OriginalCost.ToString());
                                if (validationResult.FailedItem != null)
                                {
                                    return validationResult;
                                }
