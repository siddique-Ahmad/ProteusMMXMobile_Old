    ...
    public class LoginPageViewModel : ViewModelBase
    {
        ...
        public async Task OnLoginTapAsync()
        {
            try
            {
                ...
                if (Convert.ToBoolean(user.servicestatus) == false && user.servicestatusmessge== "Password are expired need to reset")
                {
                    UserDialogs.Instance.HideLoading();

                    await DialogService.ShowAlertAsync(user.servicestatusmessge, "Alert", "OK");
