...
{
    public class ToolNumberListSelectionPageViewModel : ViewModelBase
    {
        ...
        ObservableCollection<ToolLookUp> _pickerItemCollection = new ObservableCollection<ToolLookUp>();

        public ObservableCollection<ToolLookUp> PickerItemCollection
        {
            get
            {
        ...
        ToolLookUp _nullItem = new ToolLookUp() { ToolID = null, ToolNumber = string.Empty };
        public ToolLookUp NullItem
        {
            get
            {
                return _nullItem;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.ToolNumberRequested);
        ...
        public ToolNumberListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IToolService ToolService)
        {
        ...
        public override async Task InitializeAsync(object navigationData)
        {
            try
            {
                ...
                if (navigationData != null)
                {
                    ...
                    this.ToolCribID = targetNavigationData.ToolCribID;
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                ServiceOutput ToolNumberResponse = null;
                ToolNumberResponse = await _toolService.GetToolNumber(this.WorkorderID.ToString(), UserID,this.ToolCribID.ToString(), PageNumber.ToString(), RowCount.ToString(), SearchText);


                if (ToolNumberResponse != null && ToolNumberResponse.workOrderWrapper != null && ToolNumberResponse.workOrderWrapper.toolLookUp != null)
                {
                    var toolnumber = ToolNumberResponse.workOrderWrapper.toolLookUp;
                    AddtoolNumberToPickerItemCollection(toolnumber);
                }
        ...
        private void AddtoolNumberToPickerItemCollection(List<ToolLookUp> toolnumber)
        {
            if (toolnumber != null && toolnumber.Count > 0)
            {
                foreach (var item in toolnumber)
                {
                    Device.BeginInvokeOnMainThread(() =>
                    {
