...
{
    public class CategoryListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.OnCategoryRequested);
        ...
        public CategoryListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IAssetModuleService assetService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                var CategoryResponse = await _assetService.GetCategory(UserID,this.SearchText);
                if (CategoryResponse != null && CategoryResponse.assetWrapper.listCategory != null
                    && CategoryResponse.assetWrapper.listCategory.Count>0)
                {

                    var category = CategoryResponse.assetWrapper.listCategory;
                    AddCategoryInCategoryCollection(category);
        ...
        private void AddCategoryInCategoryCollection(List<ComboDD> category)
        {
