...
namespace ProteusMMX.ViewModel.SelectionListPagesViewModels.Asset
{
    public class RuntimeUnitListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        ComboDD _nullItem = new ComboDD() { SelectedText = string.Empty, SelectedValue = null };
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.OnRuntimeUnitRequested);
        ...
        public RuntimeUnitListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IAssetModuleService assetService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
             //   OperationInProgress = true;
                var RuntimeunitResponse = await _assetService.GetRuntimeUnit(UserID, this.SearchText);
                if (RuntimeunitResponse != null && RuntimeunitResponse.assetWrapper.listRuntimeUnits != null
                    && RuntimeunitResponse.assetWrapper.listRuntimeUnits.Count > 0)
                {

                    var RU = RuntimeunitResponse.assetWrapper.listRuntimeUnits;
                    AddRuntimeInRuntimeunitCollection(RU);
        ...
        private void AddRuntimeInRuntimeunitCollection(List<ComboDD> runtimeUnit)
        {
            if (runtimeUnit != null && runtimeUnit.Count > 0)
            {
                foreach (var item in runtimeUnit)
                {
                    Device.BeginInvokeOnMainThread(() =>
                    {
