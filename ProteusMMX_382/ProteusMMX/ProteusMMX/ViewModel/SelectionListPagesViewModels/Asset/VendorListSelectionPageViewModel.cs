...
{
    public class VendorListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.OnVendorRequested);
        ...
        public VendorListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IAssetModuleService assetService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
             //   OperationInProgress = true;
                var VendorResponse = await _assetService.GetVendor(UserID,"0","0", this.SearchText);
                if (VendorResponse != null && VendorResponse.assetWrapper.listVendor != null
                    && VendorResponse.assetWrapper.listVendor.Count > 0)
                {

                    var vendor = VendorResponse.assetWrapper.listVendor;
                    AddVendorInVendorCollection(vendor);
        ...
        private void AddVendorInVendorCollection(List<ComboDD> vendor)
        {
