...
namespace ProteusMMX.ViewModel.SelectionListPagesViewModels
{
    ...
    public class PriorityListSelectionPageViewModel : ViewModelBase
    {
        #region Fields

        protected readonly IAuthenticationService _authenticationService;
        protected readonly IFormLoadInputService _formLoadInputService;
        protected readonly IPriorityService _priorityService;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.PriorityRequested);
        ...
        public PriorityListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IPriorityService priorityService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
                //OperationInProgress = true;
                ServiceOutput priorityResponse = null;
                priorityResponse = await _priorityService.GetPriorities( PageNumber.ToString(), RowCount.ToString(), SearchText);
                if (priorityResponse != null && priorityResponse.PrioritiesDD != null)
                {
                    var priorities = priorityResponse.PrioritiesDD;
                    await AddItemsToPickerItemCollection(priorities);
                }
