...
namespace ProteusMMX.ViewModel.SelectionListPagesViewModels
{
    ...
    public class ShiftListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.ShiftRequested);
        ...
        public ShiftListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IShiftService shiftService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
               // OperationInProgress = true;
                ServiceOutput shiftResponse = null;
                shiftResponse = await _shiftService.GetShifts(UserID,SearchText);
                if (shiftResponse != null && shiftResponse.ShiftsDD != null)
                {
                    var shifts = shiftResponse.ShiftsDD;
                    await AddItemsToPickerItemCollection(shifts);
                }
