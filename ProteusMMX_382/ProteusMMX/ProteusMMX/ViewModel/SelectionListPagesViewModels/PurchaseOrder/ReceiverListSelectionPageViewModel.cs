...
{
    public class ReceiverListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        ObservableCollection<POReceiver> _pickerItemCollection = new ObservableCollection<POReceiver>();

        public ObservableCollection<POReceiver> PickerItemCollection
        {
            get
            {
        ...
        POReceiver _nullItem = new POReceiver() { ReceiverID = 0, ReceiverName = string.Empty };
        public POReceiver NullItem
        {
            get
            {
                return _nullItem;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.ReceiverRequested);
        ...
        public ReceiverListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IPurchaseOrderService purchaseOrderService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
               // OperationInProgress = true;
                var Receivers = await _purchaseOrderService.GetReceivers();
                if (Receivers != null && Receivers.poWrapper != null
                    && Receivers.poWrapper.receivers != null && Receivers.poWrapper.receivers.Count > 0)
                {

                    var receivers = Receivers.poWrapper.receivers;
                    AddReceiversInCollection(receivers);
        ...
        private void AddReceiversInCollection(List<POReceiver> receivers)
        {
