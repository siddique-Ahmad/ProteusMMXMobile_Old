...
{
    public class InventoryCheckoutTOListSelectionPageViewModel:ViewModelBase
    {
        #region Fields
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.OnCheckoutRequested);
        ...
        public InventoryCheckoutTOListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IInventoryService inventoryService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                ServiceOutput PerformBY = null;
                PerformBY = await _inventoryService.GetPerformBY(UserID);
                if (PerformBY != null && PerformBY.UserComboDD != null)
                {
                    var performBY = PerformBY.UserComboDD;
                    await AddItemsToPickerItemCollection(performBY);
                }
        ...
        private async Task AddItemsToPickerItemCollection(List<ComboDD> performBY)
        {
            if (performBY != null && performBY.Count > 0)
            {
                foreach (var item in performBY)
                {
                    Device.BeginInvokeOnMainThread(() =>
                    {
