...
{
    public class TransactionReasonListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        ObservableCollection<TransactionReason> _pickerItemCollection = new ObservableCollection<TransactionReason>();

        public ObservableCollection<TransactionReason> PickerItemCollection
        {
            get
            {
        ...
        TransactionReason _nullItem = new TransactionReason() { TransactionReasonID = 0, TransactionReasonName = string.Empty };
        public TransactionReason NullItem
        {
            get
            {
                return _nullItem;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.TransactionReasonRequested);
        ...
        public TransactionReasonListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IInventoryService inventoryService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
           //     OperationInProgress = true;
                var TransactionReasonResponse = await _inventoryService.GetTransactionReason(StockroompartID.ToString(), UserID);
                if (TransactionReasonResponse != null && TransactionReasonResponse.inventoryWrapper.trasactionDialog != null
                    && TransactionReasonResponse.inventoryWrapper.trasactionDialog.transactionReasons != null && TransactionReasonResponse.inventoryWrapper.trasactionDialog.transactionReasons.Count > 0)
                {

                    var trasactionReason = TransactionReasonResponse.inventoryWrapper.trasactionDialog.transactionReasons;
                    AddTransactionReasonInCollection(trasactionReason);
        ...
        private void AddTransactionReasonInCollection(List<TransactionReason> transactionReason)
        {
            if (transactionReason != null && transactionReason.Count > 0)
            {
                foreach (var item in transactionReason)
                {
                    Device.BeginInvokeOnMainThread(() =>
                    {
