...
{
    public class InventoryShelfBinListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        public InventoryShelfBinListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IInventoryService inventoryService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                var ShelfBinResponse = await _inventoryService.GetShelfBin(StockroompartID.ToString(), UserID);
                if (ShelfBinResponse != null && ShelfBinResponse.inventoryWrapper.trasactionDialog != null
                    && ShelfBinResponse.inventoryWrapper.trasactionDialog.shelfBins != null && ShelfBinResponse.inventoryWrapper.trasactionDialog.shelfBins.Count > 0)
                {

                    var shelfbin = ShelfBinResponse.inventoryWrapper.trasactionDialog.shelfBins;
                    AddShelfbinInShelfbinCollection(shelfbin);
