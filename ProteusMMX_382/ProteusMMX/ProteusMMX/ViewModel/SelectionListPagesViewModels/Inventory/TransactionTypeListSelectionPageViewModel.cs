...
{
    public class TransactionTypeListSelectionPageViewModel:ViewModelBase
    {
        #region Fields
        ...
        ObservableCollection<TransactionType> _pickerItemCollection = new ObservableCollection<TransactionType>();

        public ObservableCollection<TransactionType> PickerItemCollection
        {
            get
            {
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.TransactionTypeRequested);
        ...
        public TransactionTypeListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IInventoryService inventoryService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                var TransactionType = await _inventoryService.GetTransactionReason(StockroompartID.ToString(), UserID);
                if (TransactionType != null && TransactionType.inventoryWrapper.trasactionDialog != null
                    && TransactionType.inventoryWrapper.trasactionDialog.transactionTypes != null && TransactionType.inventoryWrapper.trasactionDialog.transactionTypes.Count > 0)
                {

                    var trasactiontype = TransactionType.inventoryWrapper.trasactionDialog.transactionTypes;
                    AddTransactionTypeInCollection(trasactiontype);
        ...
        private void AddTransactionTypeInCollection(List<TransactionType> transactionType)
        {
