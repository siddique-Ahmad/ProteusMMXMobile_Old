...
{
     public  class InventoryPerformByListSelectionPageViewModel:ViewModelBase
     {
        #region Fields
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.OnPerformBYRequested);
        ...
        public InventoryPerformByListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IInventoryService inventoryService)
        {
