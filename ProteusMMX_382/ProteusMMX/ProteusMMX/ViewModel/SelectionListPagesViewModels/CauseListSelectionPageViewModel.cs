...
namespace ProteusMMX.ViewModel.SelectionListPagesViewModels
{
    ...
    public class CauseListSelectionPageViewModel : ViewModelBase
    {
        #region Fields

        protected readonly IAuthenticationService _authenticationService;
        protected readonly IFormLoadInputService _formLoadInputService;
        protected readonly ICauseService _causeService;
        ...
        ObservableCollection<Cause> _pickerItemCollection = new ObservableCollection<Cause>();

        public ObservableCollection<Cause> PickerItemCollection
        {
            get
            {
        ...
        Cause _nullItem = new Cause() { CauseID = null, CauseNumber = string.Empty };
        public Cause NullItem
        {
            get
            {
                return _nullItem;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.CauseRequested);
        ...
        public CauseListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, ICauseService causeService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
                //OperationInProgress = true;
                ServiceOutput causeResponse = null;
                causeResponse = await _causeService.GetCause(SearchText);
                if (causeResponse != null && causeResponse.workOrderWrapper != null && causeResponse.workOrderWrapper.Cause != null)
                {
                    var cause = causeResponse.workOrderWrapper.Cause;
                    await AddItemsToPickerItemCollection(cause);
                }
        ...
        private async Task AddItemsToPickerItemCollection(List<Cause> costCenter)
        {
