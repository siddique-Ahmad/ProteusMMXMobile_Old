...
{
    public class AssetListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        ObservableCollection<TAsset> _pickerItemCollection = new ObservableCollection<TAsset>();

        public ObservableCollection<TAsset> PickerItemCollection
        {
            get
            {
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item, MessengerKeys.AssetRequested);
        ...
        public AssetListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService, IAssetService assetService)
        {
        ...
        async Task GetPickerItems()
        {
            ...
            {
               // OperationInProgress = true;
                ServiceOutput assetResponse = null;
                if (LocationID == null)
                    assetResponse = await _assetService.GetAssetsByFacility(UserID, PageNumber.ToString(), RowCount.ToString(), FacilityID, SearchText);
                else
                    assetResponse = await _assetService.GetAssetsByFacilityAndLocation(UserID, PageNumber.ToString(), RowCount.ToString(), FacilityID, LocationID, SearchText);

                if (assetResponse != null && assetResponse.targetWrapper != null && assetResponse.targetWrapper.assets != null)
                {
                    var assets = assetResponse.targetWrapper.assets;
                    await AddFacilittToPickerItemCollection(assets);
                }
        ...
        private async Task AddFacilittToPickerItemCollection(List<TAsset> facilities)
        {
