...
{
    public class LocationListSelectionPageViewModel : ViewModelBase
    {
        #region Fields
        ...
        ObservableCollection<TLocation> _pickerItemCollection = new ObservableCollection<TLocation>();

        public ObservableCollection<TLocation> PickerItemCollection
        {
            get
            {
        ...
        TLocation _nullItem = new TLocation() { LocationID = null, LocationName = string.Empty };
        public TLocation NullItem
        {
            get
            {
                return _nullItem;
        ...
        private async void OnItemSelectedAsync(object item)
        {
            if (item != null)
            {
                MessagingCenter.Send(item , MessengerKeys.LocationRequested );
        ...
        public LocationListSelectionPageViewModel(IAuthenticationService authenticationService, IFormLoadInputService formLoadInputService , ILocationService locationService)
        {
        ...
        public override async Task InitializeAsync(object navigationData)
        {
            try
            {
                ...
                this.FacilityID = Convert.ToInt32(navigationData);
        ...
        async Task GetPickerItems()
        {
            ...
            {
              //  OperationInProgress = true;
                var locationResponse =  await _locationService.GetLocations(UserID, PageNumber.ToString(), RowCount.ToString() , FacilityID ,SearchText);
                if (locationResponse != null && locationResponse.targetWrapper != null && locationResponse.targetWrapper.locations != null)
                {
                    var locations = locationResponse.targetWrapper.locations;
                    await AddFacilittToPickerItemCollection(locations);
                }
        ...
        private async Task AddFacilittToPickerItemCollection(List<TLocation> facilities)
        {
